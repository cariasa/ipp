...
An initial statistical analysis using the methods proposed by \cite{klimek2012} showed that there is evidence of what they called incremental fraud, but there is no evidence of extreme fraud. \cite{klimek2012} consider incremental fraud to be when voting boxes have valid votes and ballots from the winning party, combining real and legal votes with votes filled on purpose to help the winning party, or by taking away votes from the other parties. On the other hand there is extreme fraud where some voting stations or districts show a 100\% turnout with 100\% count for the winning party. In Figure \ref{fig:fingerprint} can be seen a smearing to the upper right corner, an election result fingerprint associated with incremental fraud. The ideal fingerprint would have a concentration in the middle of the plane, without a correlation that leads the smearing in any direction. It is worthy to note that when extreme fraud is detected, there are clear clusters of points near the upper right corner, a behaviour that is not seen in the case of Honduras 2013 government elections.
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.4\linewidth]{images/fingerprint}
  \caption{A subfigure}
  \label{fig:fingerprint}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
